...
function formatEmptyRow(arr, key) {
  ...
  if (arr.length == 1) {
    ...
    for (var i = 0; i < arr.length; i++) {
      n = arr[i];
      if (i == 0) {
        html += `<div class="arr arr-start" onDrop="handleDrop(event)" onDragOver="allowDrop(event)" id="arr-box-${key}-${i}"><div class="num-slot" draggable="true"></div></div>`;
      } else if (i + 1 == arr.length) {
        html += `<div class="arr arr-end" onDrop="handleDrop(event)" onDragOver="allowDrop(event)" id="arr-box-${key}-${i}"><div class="num-slot" draggable="true"></div></div>`;
      } else {
        html += `<div class="arr" onDrop="handleDrop(event)" onDragOver="allowDrop(event)" id="arr-box-${key}-${i}"><div class="num-slot" draggable="true"></div></div>`;
      }
    }
  }
